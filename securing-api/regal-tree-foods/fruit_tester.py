...
try:
    ...
    print "Test 1 PASSED: Successfully made a new user"
...
try:
    ...
    print "Test 2 PASSED: Successfully obtained token!"
...
try:
    ...
    print "Test 3 PASSED: Successfully added new products"


#TEST 4: TRY ACCESSING ENDPOINT WITH AN INVALID TOKEN
try:
    h = Http()
    token = 'uuhfsvn&RWDSb788Njk'
    h.add_credentials(token, 'blank')
    url = address + '/products'
    resp, content = h.request(url,'GET' , headers = {"Content-Type" : "application/json"})
    if resp['status'] == '201' or resp['status'] == '200':
        raise Exception('Received a successful status code of %s' % resp['status'])
except Exception as err:
    print "Test 4 FAILED: Access to endpoint with valid token granted"
    print err.args
    sys.exit()
else:
    print "Test 4 PASSED: Invalid tokens were rejected!"
